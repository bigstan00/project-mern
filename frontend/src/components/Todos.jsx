...
function Todos({data, setData}) {
    ...
    const Fetch = async () => {
        setLoading(true);
        const todo = await axios.get("http://18.117.73.3/todo")
    ...
    const moveTodo = async (fromIndex, toIndex) => {
        ...
        try {
            await axios.post("http://18.117.73.3/rearrange-todos", {newOrder: updatedData})
            setData(updatedData);
        } catch (error) {
            console.error(error);
            setData(prevData);
    ...
    const clearComplete = async () => {
        try {
            let response = await axios.delete("http://18.117.73.3:3500/complete/todo")
